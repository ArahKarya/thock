...
All bundled packs (`thock/`, `clicky/`, `tactile/`, `typewriter/`, `silent/`,
`heavy/`) are **procedurally synthesized** by `tools/gen_sounds.py` (filtered
noise + decaying resonant bodies), so they are original work released under
CC0-1.0. Their `pack.json` manifests are generated by the same script.
